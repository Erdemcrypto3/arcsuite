...
// ArcScan exposes a Blockscout-compatible API
const BLOCKSCOUT_API = 'https://testnet.arcscan.app/api';
// Fee paid in native USDC (Arc gas token, 18-decimal native unit)
...
async function verifyPayment(txHash) {
  try {
    ...
    const valueUsdc = Number(tx.value) / 1e18;
    if (valueUsdc < GENERATION_FEE * 0.95) return { valid: false, reason: `Insufficient payment: ${valueUsdc} USDC` };
