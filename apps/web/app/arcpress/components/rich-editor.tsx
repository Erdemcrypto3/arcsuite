...
export function RichEditor({ value, onChange, placeholder }: Props) {
  ...
  const btn = (label: string, cmd: string, arg?: string, extra?: string) => (
    <button
      ...
      className={`rounded px-2 py-1 text-xs text-arc-600 hover:bg-purple-100 ${extra ?? ''}`}
      title={label}
    >
      {label}
    </button>
    ...
    <div>
      <div className="flex flex-wrap gap-1 mb-1 rounded-t-lg border border-purple-200 bg-purple-50/30 px-2 py-1.5">
        ...
        <button
          ...
          className="rounded px-2 py-1 text-xs text-arc-600 hover:bg-purple-100"
          title="Insert link"
        >
          Link
        </button>
      ...
      <div
        ...
        className="prose max-w-none min-h-[24rem] rounded-b-lg border border-t-0 border-purple-200 bg-white px-4 py-3 text-sm text-arc-900 focus:border-arc-500 focus:outline-none [&[data-placeholder]:empty:before]:content-[attr(data-placeholder)] [&[data-placeholder]:empty:before]:text-arc-300"
      />
    </div>
  );
}
