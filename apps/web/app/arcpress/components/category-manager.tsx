  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-arc-900">Manage Categories</h2>
          <p className="mt-1 text-xs text-arc-500">{categories.length} main categories · {articles.length} total articles</p>
        </div>
        <button onClick={onClose} className="rounded-lg bg-purple-100 px-4 py-2 text-sm font-semibold text-arc-700 ring-1 ring-inset ring-purple-200 hover:bg-purple-200">
          ← Back
        </button>
      </div>
            ...
            <div key={mi} className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-semibold text-arc-900">{cat.main}</h3>
                  <span className="rounded-full bg-arc-100 px-2 py-0.5 text-[10px] font-mono text-arc-600">{mainCount} articles</span>
                </div>
              ...
              <div className="ml-4 space-y-1">
                {cat.subs.map((sub, si) => {
                  ...
                  return (
                    <div key={si} className="flex items-center justify-between rounded bg-arc-50 px-3 py-1.5">
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-arc-700">{sub}</span>
                        <span className="rounded-full bg-purple-100 px-1.5 py-0.5 text-[9px] font-mono text-arc-500">{subCount}</span>
                      </div>
                ...
                {editingMain === mi ? (
                  <div className="flex gap-2 mt-2">
                    <input value={newSub} onChange={(e) => setNewSub(e.target.value)}
                      ...
                      className="flex-1 rounded border border-purple-200 bg-arc-50 px-3 py-1.5 text-xs focus:border-arc-500 focus:outline-none"
                      onKeyDown={(e) => e.key === 'Enter' && addSubCategory(mi)} />
                    <button onClick={() => addSubCategory(mi)} className="rounded bg-arc-500 px-3 py-1.5 text-xs text-white">Add</button>
                    <button onClick={() => { setEditingMain(null); setNewSub(''); }} className="text-xs text-arc-400">Cancel</button>
                  </div>
                ) : (
                  <button onClick={() => setEditingMain(mi)} className="mt-1 text-xs text-arc-500 hover:text-arc-700">+ Add subcategory</button>
                )}
              </div>
            </div>
          );
      ...
      <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
        <h3 className="mb-2 text-sm font-semibold text-arc-900">Add Main Category</h3>
        <div className="flex gap-2">
          <input value={newMain} onChange={(e) => setNewMain(e.target.value)}
            placeholder="Category name..."
            className="flex-1 rounded-lg border border-purple-200 bg-arc-50 px-4 py-2 text-sm focus:border-arc-500 focus:outline-none"
            onKeyDown={(e) => e.key === 'Enter' && addMainCategory()} />
          <button onClick={addMainCategory} className="rounded-lg bg-arc-500 px-4 py-2 text-sm font-semibold text-white hover:bg-arc-600">Add</button>
        </div>
      </div>

      <button onClick={resetDefaults} className="text-xs text-arc-400 hover:text-arc-600">Reset to default categories</button>
    </div>
  );
}
