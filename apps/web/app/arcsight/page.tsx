...
function getDeactivated(): number[] {
  ...
  try { return JSON.parse(localStorage.getItem('arcsight-deactivated-cats') || '[]'); } catch { return []; }
}
...
function TimeLeft({ deadline }: { deadline: bigint }) {
  ...
  if (days > 0) return <span className="text-arc-500 font-medium">{days}d {hours}h left</span>;
...
function CategoryBadges({ mask }: { mask: number }) {
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {indices.map((i) => (
        <span key={i} className="px-2 py-0.5 text-xs rounded-full bg-arc-100 text-arc-700 font-medium">
          {CATEGORIES[i]}
        </span>
      ))}
    </div>
...
function StatCard({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="bg-white rounded-xl ring-1 ring-inset ring-purple-100 p-4 text-center">
      <div className="text-2xl font-bold text-arc-600">{value}</div>
      <div className="text-xs text-arc-400 mt-1">{label}</div>
    </div>
  );
}
...
function RegisterPanel({ user, onRegistered }: { user: UserProfile | null; onRegistered: () => void }) {
  ...
  return (
    <div className="bg-white rounded-xl ring-1 ring-inset ring-purple-100 p-6 shadow-sm max-w-lg mx-auto">
      ...
      <p className="text-sm text-arc-500 mb-4 leading-relaxed">
        ...
        {CATEGORIES.map((cat, i) => !isCategoryActive(i) ? null : (
          <button
            ...
            className={`px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
              selected.includes(i)
                ? 'bg-arc-500 text-white ring-2 ring-arc-300'
                : 'bg-arc-50 text-arc-600 hover:bg-arc-100'
            }`}
          >
            {cat}
          </button>
      ...
      <button
        ...
        className="w-full py-3 rounded-xl bg-arc-500 text-white font-semibold hover:bg-arc-600 disabled:opacity-50 transition-all"
      >
        {isPending ? 'Confirming...' : user?.registered ? 'Update Categories' : 'Register (+50 points)'}
      </button>
      {user?.registered && (
        <p className="text-center text-xs text-arc-400 mt-3">
          Your points: <span className="font-bold text-arc-600">{user.points.toString()}</span> |
          Streak: <span className="font-bold text-arc-600">{user.streak.toString()}</span>
        </p>
      )}
    </div>
  );
...
function PollCard({ poll, userResponse, results, onRespond, isPending }: {
  ...
  return (
    <div className="bg-white rounded-xl ring-1 ring-inset ring-purple-100 p-5 shadow-sm">
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className={`w-2 h-2 rounded-full ${responded ? 'bg-green-400' : expired ? 'bg-red-400' : 'bg-arc-400'}`} />
            <span className="text-xs text-arc-400">Poll #{poll.id}</span>
      ...
      <p className="text-sm text-arc-700 mb-4 font-medium">{poll.contentCID}</p>
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-lg text-sm transition-all relative overflow-hidden ${
                isSelected
                  ...
                  : 'bg-arc-50 text-arc-700 hover:bg-arc-100 cursor-pointer'
              }`}
            >
              {responded && (
                <div className="absolute inset-y-0 left-0 bg-arc-200 opacity-30" style={{ width: `${pct}%` }} />
              )}
      ...
      {!responded && !expired && (
        <button
          ...
          className="w-full py-2.5 rounded-xl bg-arc-500 text-white font-semibold hover:bg-arc-600 disabled:opacity-50 text-sm transition-all"
        >
      ...
      <div className="flex justify-between text-xs text-arc-400 mt-3 pt-3 border-t border-purple-50">
  ...
  return (
    <div className="max-w-lg mx-auto">
      <h2 className="text-xl font-bold mb-4">
        My Inbox <span className="text-arc-400 text-sm font-normal">({ids.length} polls)</span>
      </h2>
      {ids.length === 0 ? (
        <div className="bg-white rounded-xl ring-1 ring-inset ring-purple-100 p-8 text-center">
          <p className="text-arc-400">No polls matching your categories yet. Check back soon!</p>
        </div>
      ) : (
...
function SendPanel() {
  ...
  return (
    <div className="bg-white rounded-xl ring-1 ring-inset ring-purple-100 p-6 shadow-sm max-w-lg mx-auto">
      ...
      <p className="text-sm text-arc-500 mb-5 leading-relaxed">
      ...
      <label className="block text-sm font-medium text-arc-600 mb-1">Your Question</label>
      <input
        value={contentCID} onChange={(e) => setContentCID(e.target.value)}
        placeholder="What do you want to ask the community?"
        className="w-full px-3 py-2 rounded-lg bg-arc-50 ring-1 ring-inset ring-purple-100 text-sm mb-4 focus:outline-none focus:ring-arc-300"
      />

      <label className="block text-sm font-medium text-arc-600 mb-1">Response Options</label>
      <div className="space-y-2 mb-4">
        {options.map((opt, i) => (
          <div key={i} className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 rounded-lg bg-arc-50 ring-1 ring-inset ring-purple-100 text-sm focus:outline-none focus:ring-arc-300"
            />
            {options.length > 2 && (
        ...
        {options.length < 10 && (
          <button onClick={addOption} className="text-sm text-arc-500 hover:text-arc-600 font-medium">+ Add option</button>
        )}
      </div>

      <label className="block text-sm font-medium text-arc-600 mb-1">Target Categories</label>
      <div className="grid grid-cols-2 gap-2 mb-4">
        {CATEGORIES.map((cat, i) => (
          <button key={cat} onClick={() => toggleCat(i)}
            className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${
              selectedCats.includes(i) ? 'bg-arc-500 text-white' : 'bg-arc-50 text-arc-600 hover:bg-arc-100'
            }`}
          >{cat}</button>
        ))}
      </div>

      <label className="block text-sm font-medium text-arc-600 mb-1">Deadline</label>
      <select
        value={deadlineDays} onChange={(e) => setDeadlineDays(Number(e.target.value))}
        className="w-full px-3 py-2 rounded-lg bg-arc-50 ring-1 ring-inset ring-purple-100 text-sm mb-5 focus:outline-none"
      >
      ...
      {mask > 0 && (
        <div className="bg-arc-50 rounded-lg p-4 mb-5 text-sm">
          <div className="flex justify-between mb-1">
            <span className="text-arc-400">Matching audience:</span>
            <span className="font-bold text-arc-700">{audienceSize?.toString() ?? '...'} wallets</span>
          </div>
          <div className="flex justify-between">
            <span className="text-arc-400">Price:</span>
            <span className="font-bold text-arc-700">
      ...
      <p className="text-xs text-arc-400 text-center mt-2">
...
function AdminPanel({ address }: { address: string }) {
  ...
  const saveDeactivated = (list: number[]) => {
    setDeactivated(list);
    localStorage.setItem('arcsight-deactivated-cats', JSON.stringify(list));
  };
  ...
  if (!authorized) {
    return (
      <div className="bg-white rounded-xl ring-1 ring-inset ring-purple-100 p-8 text-center max-w-lg mx-auto">
        <p className="text-arc-400">Admin access required.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl ring-1 ring-inset ring-purple-100 p-5 shadow-sm">
        ...
        <p className="text-xs text-arc-400 mb-3">On-chain categories. Users select these during registration. Polls target specific categories.</p>
        <div className="space-y-2 mb-4">
          {categories.map((cat, i) => {
            const mask = 1 << i;
        ...
        <div className="flex gap-2">
          <input value={newCatName} onChange={(e) => setNewCatName(e.target.value)}
            ...
            className="flex-1 px-3 py-2 rounded-lg bg-arc-50 ring-1 ring-inset ring-purple-100 text-sm focus:outline-none"
            onKeyDown={(e) => e.key === 'Enter' && addCategoryOnChain()} />
          <button onClick={addCategoryOnChain} disabled={isPending || !newCatName.trim()}
            className="px-4 py-2 rounded-lg bg-arc-500 text-white text-sm font-medium hover:bg-arc-600 disabled:opacity-50">
            Add Category
          </button>
        </div>
        <p className="mt-2 text-[10px] text-arc-400">Adding a category requires an on-chain transaction. Categories cannot be removed once added.</p>
      </div>

      {/* Poll Actions */}
      <div className="bg-white rounded-xl ring-1 ring-inset ring-purple-100 p-5 shadow-sm">
        <h3 className="font-bold mb-3">Poll Actions</h3>
        <label className="text-xs font-medium text-arc-400 block mb-1">Poll ID</label>
        <input
          type="number"
          value={generateInput}
          onChange={(e) => setGenerateInput(e.target.value)}
          placeholder="Enter poll ID"
          className="w-full px-3 py-2 rounded-lg bg-arc-50 ring-1 ring-inset ring-purple-100 text-sm mb-3 focus:outline-none"
        />
        <div className="flex gap-2">
          <button
          ...
          <button
            ...
            disabled={isPending || !generateInput}
            className="flex-1 py-2.5 rounded-lg bg-arc-500 text-white text-sm font-medium hover:bg-arc-600 disabled:opacity-50"
          >Close</button>
        </div>
      </div>

      {isPending && (
        <div className="fixed bottom-4 right-4 bg-arc-500 text-white px-4 py-2 rounded-lg text-sm shadow-lg">
          Transaction pending...
        </div>
      )}
    </div>
...
function CategoryRow({ index, name, mask, isDeactivated, onToggle }: { index: number; name: string; mask: number; isDeactivated: boolean; onToggle: () => void }) {
  ...
  return (
    <div className={`flex items-center justify-between rounded-lg px-3 py-2 ${isDeactivated ? 'bg-red-50 opacity-60' : 'bg-arc-50'}`}>
      <div className="flex items-center gap-2">
        <span className="text-sm font-medium text-arc-700">{name}</span>
        <span className="rounded-full bg-purple-100 px-2 py-0.5 text-[9px] font-mono text-arc-500">bit {index}</span>
        {isDeactivated && <span className="rounded-full bg-red-100 px-2 py-0.5 text-[9px] font-semibold text-red-700">Hidden</span>}
      </div>
      <div className="flex items-center gap-3">
        <span className="text-xs font-mono text-arc-500">{count} users</span>
...
function LeaderboardPanel() {
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <p className="text-sm text-arc-400 mb-4">
      ...
      <div className="bg-white rounded-xl ring-1 ring-inset ring-purple-100 shadow-sm overflow-hidden">
        {addresses.length === 0 ? (
          <div className="p-8 text-center text-arc-400">No users yet.</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-arc-50 text-arc-400 text-xs">
            ...
            <tbody>
              {addresses.map((addr, i) => (
                <tr key={addr} className="border-t border-purple-50 hover:bg-arc-50/50">
                  <td className="py-3 px-4 font-bold text-arc-500">{i + 1}</td>
                  <td className="py-3 px-4 font-mono text-xs">{addr}</td>
                  <td className="py-3 px-4 text-right font-bold text-arc-600">{points[i].toString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="min-h-screen">
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-purple-100 sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <a href="https://arcsuite.xyz" className="inline-flex items-center gap-1 rounded-lg bg-purple-100 px-3 py-1.5 text-xs font-semibold text-arc-700 ring-1 ring-inset ring-purple-200 hover:bg-purple-200 hover:text-arc-900">← arcsuite.xyz</a>
            <h1 className="text-lg font-bold text-arc-700">ArcSight</h1>
            {user?.registered && (
              <span className="text-xs bg-arc-100 text-arc-600 px-2 py-0.5 rounded-full font-medium">
      ...
      {!isConnected ? (
        <div className="max-w-lg mx-auto mt-20 text-center px-4">
          <h2 className="text-3xl font-bold text-arc-700 mb-3">ArcSight</h2>
          <p className="text-arc-400 mb-2">Community insight platform for Arc L1</p>
          <p className="text-sm text-arc-300 mb-8">
              ...
              {tabs.map((t) => (
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
                    tab === t.id ? 'bg-arc-500 text-white shadow-sm' : 'text-arc-400 hover:text-arc-600 hover:bg-arc-50'
                  }`}
                >
                  {t.label}
                </button>
      ...
      {/* Footer */}
      <footer className="mt-auto py-6 text-center text-xs text-arc-300">
        <a href="https://arcsuite.xyz" className="hover:text-arc-500">Arc Suite</a>
        {' '}&middot;{' '}Built on Arc L1
      </footer>
    </div>
  );
}
