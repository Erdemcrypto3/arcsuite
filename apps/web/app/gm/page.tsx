    ...
    return (
      <div className="flex flex-col items-center gap-8 py-16">
        <div className="text-center">
          ...
          <p className="text-lg text-arc-600">Connect your wallet to say gm on-chain</p>
          <p className="mt-2 text-sm text-arc-500">A 0-value self-transfer with &quot;gm&quot; as calldata. Costs only gas.</p>
        </div>
        <ConnectButton />
      </div>
    );
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col items-center gap-6">
        <button
          ...
          className={`group relative flex h-40 w-40 items-center justify-center rounded-full text-5xl font-bold shadow-lg transition-all ${
            alreadySaidGm
              ...
              : 'bg-arc-500 text-white hover:bg-arc-600 hover:scale-105 active:scale-95 cursor-pointer'
          }`}
        >
          ...
          <div className="text-center">
            ...
            {txHash && (
              <a
                ...
                className="text-sm text-arc-500 hover:text-arc-600 underline"
              >
                View transaction
              </a>
            )}
        ...
        {!alreadySaidGm && !isSending && !isConfirming && (
          <p className="text-sm text-arc-500">Send a 0-value tx to yourself with &quot;gm&quot; as calldata</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-xl bg-white p-4 text-center ring-1 ring-inset ring-purple-100 shadow-sm">
          <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">Current Streak</div>
          <div className="mt-1 font-mono text-3xl font-bold text-arc-500">
            {gmData.currentStreak}
          </div>
          <div className="text-xs text-arc-400">days</div>
        </div>
        <div className="rounded-xl bg-white p-4 text-center ring-1 ring-inset ring-purple-100 shadow-sm">
          <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">Longest Streak</div>
          <div className="mt-1 font-mono text-3xl font-bold text-arc-900">
            {gmData.longestStreak}
          </div>
          <div className="text-xs text-arc-400">days</div>
        </div>
        <div className="rounded-xl bg-white p-4 text-center ring-1 ring-inset ring-purple-100 shadow-sm">
          <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">Total GMs</div>
          <div className="mt-1 font-mono text-3xl font-bold text-arc-900">
            {gmData.totalGms}
          </div>
          <div className="text-xs text-arc-400">on-chain</div>
        </div>
      </div>
    ...
    <ArcWalletProvider>
      <main className="mx-auto max-w-lg px-6 py-10 sm:py-16">
        <header className="mb-8">
          ...
          <h1 className="text-3xl font-semibold tracking-tight text-arc-900 sm:text-4xl">
            GM Widget
          </h1>
