...
export function QuizSession({ questions, onComplete, onBack }: Props) {
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        <div className="rounded-xl bg-white p-8 text-center ring-1 ring-inset ring-purple-100 shadow-sm">
          <h2 className="text-2xl font-semibold text-arc-900">Quiz Complete!</h2>
          <div className="mt-4 font-mono text-5xl font-bold text-arc-500">{totalPoints}</div>
          <div className="mt-2 text-arc-600">points earned</div>
          <div className="mt-4 text-sm text-arc-700">
            {correctCount}/{questions.length} correct
          </div>
          <div className="mt-6 flex justify-center gap-3">
            <button
              onClick={onBack}
              className="rounded-lg bg-purple-100 px-5 py-2.5 text-sm font-semibold text-arc-700 ring-1 ring-inset ring-purple-200 hover:bg-purple-200"
            >
              Back to Categories
            </button>
          </div>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      {/* Progress bar */}
      <div className="flex items-center justify-between text-sm text-arc-600">
        <button
          onClick={onBack}
          className="text-arc-500 hover:text-arc-700"
        >
          ← Quit
        </button>
        <span>
      ...
      <div className="rounded-xl bg-white p-6 ring-1 ring-inset ring-purple-100 shadow-sm">
        <p className="text-lg font-semibold text-arc-900 leading-relaxed">{q.question}</p>
      ...
      <div className="space-y-2">
        {q.options.map((option, idx) => {
          let style = 'bg-white ring-purple-200 hover:bg-purple-50 cursor-pointer text-arc-900';
          if (isAnswered) {
            if (idx === q.correctIndex) {
              ...
              style = 'bg-white/50 ring-purple-100 text-arc-400 cursor-not-allowed';
            }
          }
          ...
          return (
            ...
            >
              <span className="mr-3 inline-block w-6 rounded bg-purple-100/50 text-center font-mono font-bold text-arc-500">
                {letter}
              </span>
              {option}
            </button>
        ...
        <div className="space-y-3">
          ...
          {!isCorrect && !showSolution && (
            <button
              ...
              className="text-sm text-arc-500 hover:text-arc-700 underline"
            >
              Show solution
            </button>
          )}
          ...
          {showSolution && (
            <div className="rounded-lg bg-purple-50 px-4 py-3 text-sm text-arc-700 ring-1 ring-inset ring-purple-200">
          ...
          <button
            onClick={handleNext}
            className="w-full rounded-lg bg-arc-500 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-arc-700"
          >
