...
import { ArcWalletProvider, ConnectButton, useAccount, useWriteContract } from '@arcsuite/wallet';
...
const API_URL = 'https://arcsuite.xyz';
...
function MintAchievementButton({ category, level, points }: { category: { id: string; label: string; icon: string }; level: number; points: number }) {
  ...
  return (
    <button onClick={handleMint} disabled={isPending || minting}
      className="rounded-lg bg-arc-500 px-3 py-1 text-[10px] font-semibold text-white hover:bg-arc-600 disabled:opacity-50">
  ...
  if (loading) {
    return (
      <main className="mx-auto max-w-4xl px-6 py-16">
        <div className="py-24 text-center text-arc-600">Loading questions…</div>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-16 sm:py-20">
      <header className="mb-10">
        <div className="mb-6 flex items-center justify-between">
          <a
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-purple-100 px-4 py-2 text-sm font-semibold text-arc-700 ring-1 ring-inset ring-purple-200 shadow-sm transition hover:bg-purple-200 hover:text-arc-900"
          >
            ← arcsuite.xyz
          </a>
      ...
      {totalQuizzes > 0 && (
        <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
            <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">Total Points</div>
            <div className="mt-1 font-mono text-2xl font-bold text-arc-500">{totalPoints}</div>
          </div>
          <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
            <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">Quizzes Taken</div>
            <div className="mt-1 font-mono text-2xl font-bold text-arc-900">{totalQuizzes}</div>
          </div>
          <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
            <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">Questions Seen</div>
            <div className="mt-1 font-mono text-2xl font-bold text-arc-900">
              {Object.values(progress).reduce((s, p) => s + p.questionsAnswered.length, 0)}
            </div>
          </div>
          <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
            <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">Achievements</div>
            <div className="mt-1 font-mono text-2xl font-bold text-arc-900">
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              className="group rounded-xl bg-white p-6 text-left ring-1 ring-inset ring-purple-100 shadow-sm transition hover:bg-purple-50 hover:ring-arc-500"
            >
              ...
              <h3 className="text-lg font-semibold text-arc-900">{cat.label}</h3>
              <p className="mt-1 text-xs text-arc-600">{catQuestionCount} questions</p>
              {p.quizzesTaken > 0 && (
                <div className="mt-3 space-y-1">
                  <div className="flex justify-between text-[10px] text-arc-500">
                    <span>{p.totalPoints} pts</span>
                    <span>{p.quizzesTaken} quizzes</span>
                  </div>
                  <div className="h-1.5 rounded-full bg-purple-100">
                    <div
                      className="h-1.5 rounded-full bg-arc-500 transition-all"
      ...
      <div className="mt-10 space-y-4">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-arc-600">
            ...
            <div key={cat.id}>
              <div className="mb-1 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-arc-700">{cat.label}</span>
                  {p.achievementLevel >= 0 && (
                    <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-700 ring-1 ring-inset ring-emerald-300">
                      {ACHIEVEMENTS[p.achievementLevel]?.name}
                    </span>
