  ...
  return (
    <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-arc-600">
          Your Squad ({squad.length}/15)
        </h3>
        <div className="flex gap-3 text-xs">
          <span className="text-arc-600">
            Budget: <span className={`font-mono font-semibold ${remaining >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{formatCost(remaining)}</span>
          </span>
          <span className="text-arc-600">
            Value: <span className="font-mono font-semibold text-arc-900">{formatCost(totalCost)}</span>
          </span>
          <span className="text-arc-600">
            Points: <span className="font-mono font-semibold text-arc-500">{totalPoints}</span>
          </span>
        </div>
      </div>
          ...
          return (
            <div key={pos}>
              <div className="mb-1 flex items-center gap-2">
                ...
                <span className="text-xs text-arc-500">
              ...
              <div className="grid grid-cols-1 gap-1">
                {players.map((p) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-arc-900">{p.web_name}</span>
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-mono text-[10px] text-arc-600">{formatCost(p.now_cost)}</span>
                      <span className="font-mono text-[10px] font-semibold text-arc-500">{p.total_points}pts</span>
                      <button
                ...
                {Array.from({ length: emptySlots }, (_, i) => (
                  <div
                    key={`empty-${pos}-${i}`}
                    className="flex items-center justify-center rounded-lg border-2 border-dashed border-purple-200 px-3 py-2 text-[10px] text-arc-400"
                  >
                    Empty {pos} slot
                  </div>
                ))}
