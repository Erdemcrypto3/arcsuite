  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-xl bg-arc-500 p-5 shadow-sm">
        ...
        <div className="rounded-xl bg-white p-5 ring-1 ring-inset ring-purple-100 shadow-sm">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {yourRank.length === 0 ? (
              <span className="text-sm text-arc-500">No squad players ranked yet.</span>
            ) : (
              yourRank.slice(0, 8).map((r) => (
                <span
                  key={r.player.id}
                  className="rounded-full bg-purple-100 px-2 py-0.5 text-xs font-semibold text-arc-700 ring-1 ring-inset ring-purple-200"
                >
      ...
      <div className="rounded-xl bg-white ring-1 ring-inset ring-purple-100 shadow-sm">
        <div className="border-b border-purple-200 px-4 py-3">
          <h3 className="text-sm font-semibold text-arc-900">Top 50 players — season points</h3>
          <p className="text-xs text-arc-500">Highlighted rows are in your squad.</p>
        </div>
            ...
            {playerTotals.map((row, i) => {
              ...
              return (
                <tr key={row.player.id} className={inSquad ? 'bg-amber-50/50' : 'hover:bg-purple-50'}>
                  <td className="px-3 py-2 font-mono text-arc-500">{i + 1}</td>
                  <td className="px-3 py-2 font-semibold text-arc-900">
                    {row.player.web_name}
                    {inSquad && (
                  ...
                  <td className="px-3 py-2 text-center text-xs text-arc-600">
                  ...
                  <td className="px-3 py-2 text-right font-mono font-bold text-arc-900">{row.total}</td>
                </tr>
              );
            })}
          </tbody>
