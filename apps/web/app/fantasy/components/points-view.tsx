    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-arc-600">Gameweek</span>
          <select
            value={selectedGW}
            onChange={(e) => setSelectedGW(Number(e.target.value))}
            className="rounded-lg bg-white px-3 py-2 text-sm text-arc-900 ring-1 ring-inset ring-purple-200 shadow-sm"
          >
            {recentGWs.map((gw) => (
        ...
        <div className="rounded-xl bg-arc-500 px-5 py-3 text-center shadow-sm">
      ...
      {gwFixtures.length > 0 && (
        <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
          <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-arc-600">
          ...
          <div className="flex flex-wrap gap-2">
            {gwFixtures.map((f) => (
              ...
              >
                <span className="font-semibold text-arc-900">
                  {teamMap.get(f.home)?.short_name ?? '?'}
                </span>
                <span className="mx-1 text-arc-500">
                  {f.finished
      ...
      {squad.length === 0 && (
        <div className="rounded-xl bg-white p-8 text-center text-arc-500 ring-1 ring-inset ring-purple-100 shadow-sm">
      ...
      {squad.length > 0 && (
        <div className="rounded-xl bg-white ring-1 ring-inset ring-purple-100 shadow-sm">
          <table className="w-full text-sm">
            <thead className="border-b border-purple-200 text-xs uppercase tracking-wider text-arc-500">
              <tr>
                    ...
                    >
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-arc-500">
                      ...
                      <td className="px-3 py-2.5 text-center font-mono text-arc-600">
                        {pp.stats?.minutes ?? '—'}
                      </td>
                      <td className="px-3 py-2.5 text-center font-mono text-arc-900">
                      ...
                      <td className="px-3 py-2.5 text-center font-mono text-arc-500">
                        {pp.stats?.bonus ?? '—'}
                      </td>
                      <td className="px-3 py-2.5 text-right font-mono font-bold text-arc-900">
                        {pp.finalPoints}
                      </td>
                      <td className="px-3 py-2.5 text-center">
                        <button
                          ...
                          className={`rounded px-2 py-1 text-[10px] font-semibold transition ${
                            pp.isCaptain
                              ...
                              : 'text-arc-400 hover:bg-purple-50 hover:text-arc-700'
                          }`}
            ...
            <tfoot className="border-t-2 border-purple-300 bg-purple-50/50">
              <tr>
                <td colSpan={8} className="px-4 py-3 text-sm font-semibold text-arc-700">
                  Total GW{selectedGW} Points
                </td>
                <td className="px-3 py-3 text-right font-mono text-xl font-bold text-arc-500">
                  {totalPoints}
                </td>
                <td />
              </tr>
      ...
      {squad.length > 0 && highestScorer && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
            <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">
              Highest scorer
            </div>
            <div className="mt-1 font-semibold text-arc-900">
              {highestScorer.player.web_name}
            </div>
            <div className="font-mono text-lg text-arc-500">{highestScorer.basePoints} pts</div>
          </div>
          <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
            <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">
              Captain bonus
            </div>
            <div className="mt-1 font-mono text-lg text-arc-900">
              +{captainId ? (gwLive[String(captainId)]?.total_points ?? 0) : 0} pts
            </div>
            <div className="text-xs text-arc-500">From ×2 captain multiplier</div>
          </div>
          <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
            <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">
              Dream Team players
            ...
            <div className="text-xs text-arc-500">★ in your squad</div>
          </div>
        </div>
      )}

      <p className="text-xs text-arc-400">
