    ...
    <div className="rounded-xl bg-white p-4 ring-1 ring-inset ring-purple-100 shadow-sm">
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        <input
          ...
          className="flex-1 min-w-[140px] rounded-lg bg-purple-50/50 px-3 py-2 text-xs text-arc-900 ring-1 ring-inset ring-purple-200 placeholder:text-arc-400 focus:outline-none focus:ring-2 focus:ring-arc-500"
        />
        <select
          value={posFilter}
          onChange={(e) => setPosFilter(e.target.value as Position | 'ALL')}
          className="rounded-lg bg-purple-50/50 px-2 py-2 text-xs text-arc-900 ring-1 ring-inset ring-purple-200"
        >
      ...
      <div className="max-h-[400px] overflow-y-auto">
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-white border-b border-purple-200 text-arc-500 uppercase tracking-wider">
            <tr>
              ...
              return (
                <tr key={p.id} className={`hover:bg-purple-50 ${disabled ? 'opacity-40' : ''}`}>
                  <td className="px-2 py-2">
                    <span className="font-semibold text-arc-900">{p.web_name}</span>
                    {p.status !== 'a' && (
                  ...
                  <td className="px-2 py-2 text-center text-arc-600">
                    {teamMap.get(p.team)?.short_name ?? '?'}
                  </td>
                  <td className="px-2 py-2 text-right font-mono text-arc-700">
                    {(p.now_cost / 10).toFixed(1)}
                  </td>
                  <td className="px-2 py-2 text-right font-mono font-semibold text-arc-900">
                    {p.total_points}
                  </td>
                  <td className="px-2 py-2 text-right font-mono text-arc-600">
                    {p.form}
                  </td>
                  <td className="px-2 py-2 text-right">
                    <button
                      ...
                      className={`rounded px-2 py-1 text-[10px] font-semibold ${
                        disabled
                          ? 'bg-purple-50 text-arc-300 cursor-not-allowed'
                          : 'bg-arc-500 text-white hover:bg-arc-700'
                      }`}
                    >
