...
const KNOWN_CONTRACTS: Record<string, string> = {
  // Add known Arc contracts here as we discover them
};
...
export default function HealthPage() {
  ...
  const revokeUrl = address.trim()
    ? `https://revoke.cash/address/${address.trim()}?chainId=5042002`
    : 'https://revoke.cash';
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10 sm:py-16">
      <header className="mb-8">
        ...
        <h1 className="text-3xl font-semibold tracking-tight text-arc-900 sm:text-4xl">Wallet Health</h1>
        <p className="mt-2 text-sm text-arc-600">
          Check token approvals for any Arc wallet. Spot risky unlimited approvals.
        </p>
      </header>

      {/* Search */}
      <div className="mb-8 rounded-xl bg-white p-6 ring-1 ring-inset ring-purple-100 shadow-sm">
        <label className="block text-xs font-semibold text-arc-600 mb-2">Wallet Address</label>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            <div className="rounded-xl bg-white p-4 text-center ring-1 ring-inset ring-purple-100 shadow-sm">
              <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">Total Txs</div>
              <div className="mt-1 font-mono text-2xl font-bold text-arc-900">{totalTxs}</div>
            </div>
            <div className="rounded-xl bg-white p-4 text-center ring-1 ring-inset ring-purple-100 shadow-sm">
              <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">Approvals Found</div>
              <div className="mt-1 font-mono text-2xl font-bold text-arc-500">{approvals.length}</div>
            </div>
            <div className="rounded-xl bg-white p-4 text-center ring-1 ring-inset ring-purple-100 shadow-sm sm:col-span-1 col-span-2">
              <div className="text-xs font-semibold uppercase tracking-wider text-arc-500">Unlimited</div>
          ...
          {approvals.length > 0 ? (
            <div>
              <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-arc-600">Token Approvals</h2>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-arc-900">
                            Token: <a href={`${EXPLORER_URL}/address/${a.tokenContract}`} target="_blank" rel="noopener noreferrer" className="font-mono text-arc-500 hover:text-arc-600 underline">{shortenAddress(a.tokenContract)}</a>
                          </span>
                        </div>
                        <div className="text-xs text-arc-500">
                          Spender: <a href={`${EXPLORER_URL}/address/${a.spender}`} target="_blank" rel="noopener noreferrer" className="font-mono hover:text-arc-600 underline">
                            {KNOWN_CONTRACTS[a.spender.toLowerCase()] || shortenAddress(a.spender)}
                          </a>
                        </div>
                        <div className="text-xs text-arc-400">{formatDate(a.timestamp)}</div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <a href={`${EXPLORER_URL}/tx/${a.hash}`} target="_blank" rel="noopener noreferrer"
                          className="text-xs text-arc-400 hover:text-arc-600 underline">tx</a>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="rounded-xl bg-emerald-50 p-8 text-center ring-1 ring-inset ring-emerald-200">
              ...
              <p className="text-sm text-emerald-700">No token approvals found for this wallet on Arc.</p>
            </div>
          )}
        </div>
      )}
      ...
      {!searched && (
        <div>
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-arc-600">Why Check Approvals?</h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-xl bg-white p-5 ring-1 ring-inset ring-purple-100 shadow-sm">
              <h3 className="text-sm font-semibold text-arc-900">Unlimited Approvals Are Risky</h3>
            ...
            <div className="rounded-xl bg-white p-5 ring-1 ring-inset ring-purple-100 shadow-sm">
              <h3 className="text-sm font-semibold text-arc-900">Regular Checkups Matter</h3>
              <p className="mt-1 text-xs text-arc-600">
            ...
            <div className="rounded-xl bg-white p-5 ring-1 ring-inset ring-purple-100 shadow-sm">
              <h3 className="text-sm font-semibold text-arc-900">No Wallet Connection Needed</h3>
              <p className="mt-1 text-xs text-arc-600">
                Just paste any Arc address to check. No wallet connection required for viewing.
                To revoke, you&apos;ll use Revoke.cash with your wallet connected.
              </p>
            </div>
            <div className="rounded-xl bg-white p-5 ring-1 ring-inset ring-purple-100 shadow-sm">
              <h3 className="text-sm font-semibold text-arc-900">Powered by Blockscout</h3>
              <p className="mt-1 text-xs text-arc-600">
