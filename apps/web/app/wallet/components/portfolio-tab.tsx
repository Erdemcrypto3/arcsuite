...
import { arcPublicClient, BLOCKSCOUT_API_URL, EXPLORER_URL } from '@arcsuite/chain';
...
// Known legitimate tokens on Arc — matched by symbol (case-insensitive)
...
export function PortfolioTab({ address }: { address: Address }) {
  ...
  if (loading) return <div className="py-12 text-center text-arc-600">Loading portfolio…</div>;
  if (error)
    return (
    ...
    <>
      ...
      {subTab === 'overview' && (
        <>
          ...
          {data.ethPrice !== null && (
            <div className="mb-6 text-xs text-arc-500">
          ...
          {data.erc20Tokens.length > 0 && (
            <section className="mb-6">
              <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-arc-600">
                Top tokens
              </h3>
                      ...
                      return (
                        <tr key={t.contractAddress} className="hover:bg-purple-50">
                          <td className="px-4 py-3">
                            ...
                            <span className="ml-2 text-arc-500">{t.name || 'Unknown'}</span>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <VerifiedBadge verified={isVerified(t.symbol)} />
                          </td>
                          <td className="px-4 py-3 text-right font-mono text-arc-900">
                            {formatBalance(t.balance, t.decimals)}
                          </td>
                          <td className="px-4 py-3 text-right font-mono text-arc-700">
                            {formatUsd(usdVal)}
                          </td>
                        </tr>
                      );
        ...
        <section>
          <div className="mb-4 rounded-xl bg-white p-5 ring-1 ring-inset ring-purple-100 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-sm text-arc-700">Native ETH</span>
                <span className="ml-2">
                  <VerifiedBadge verified={true} />
                </span>
              </div>
              <div className="text-right">
                <span className="font-mono text-lg font-semibold text-arc-900">
                  {formatEthBalance(data.ethBalanceWei)}{' '}
                  <span className="text-sm font-normal text-arc-500">ETH</span>
                </span>
                {ethUsdValue !== null && (
                  <div className="text-xs font-mono text-arc-500">{formatUsd(ethUsdValue)}</div>
                )}
              </div>
            </div>
          </div>
          ...
          {data.erc20Tokens.length === 0 ? (
            <div className="py-12 text-center text-arc-500">
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3">
                          ...
                          <div className="mt-0.5 font-mono text-[10px] text-arc-400">
          ...
          <p className="mt-3 text-xs text-arc-400">
              ...
              {data.nftTokens.map((t) => (
                ...
                >
                  <div className="mb-2 flex items-start justify-between gap-2">
                    <div>
                      <div className="font-semibold text-arc-900">
                        {t.name || 'Unknown NFT'}
                      </div>
                    ...
                    <span className="inline-flex items-center rounded-full bg-purple-100 px-2 py-0.5 text-xs font-medium text-arc-700 ring-1 ring-inset ring-purple-300">
                      ×{t.balance || '1'}
                    </span>
                  </div>
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-xs text-arc-500 hover:underline"
                    >
                      view →
                    </a>
                  </div>
