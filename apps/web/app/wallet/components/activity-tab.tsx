...
export function ActivityTab({ address }: { address: Address }) {
  ...
  if (loading) return <div className="py-12 text-center text-arc-600">Loading activity…</div>;
  if (error) return (
  ...
  return (
    <>
      ...
      {metrics.topContracts.length > 0 && (
        <section>
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-arc-600">Top contracts</h2>
              ...
              <tbody className="divide-y divide-purple-100 font-mono">
                {metrics.topContracts.map((c) => (
                  <tr key={c.address} className="hover:bg-purple-50">
                    <td className="px-4 py-3 text-arc-900">{shortAddress(c.address)}</td>
                    <td className="px-4 py-3 text-right text-arc-700">{c.txCount}</td>
                    <td className="px-4 py-3 text-right">
                      <a href={`https://explorer.arc.network/address/${c.address}`} target="_blank" rel="noopener noreferrer" className="text-arc-500 hover:underline">open →</a>
                    </td>
                  </tr>
                ))}
              </tbody>
