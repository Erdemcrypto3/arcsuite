...
export function Checkers() {
  ...
  const statusColor = () => {
    ...
    if (turn === 'light') return 'text-arc-500';
    return 'text-arc-900';
  };
    ...
    <div className="flex flex-col items-center gap-6 py-2 select-none">
      ...
      <div className="flex w-full max-w-lg items-center justify-between rounded-xl bg-white px-5 py-3 ring-1 ring-inset ring-purple-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-arc-500 mt-0.5">Move {moveCount}</p>
          {status === 'dark-wins' && (
            <MintScoreNFTButton gameId="checkers" gameTitle="Checkers" gameIcon="♟️" score={capturedDark * 10 + Math.max(0, 60 - moveCount)} />
          )}
        </div>
        <div className="flex items-center gap-4 text-xs text-arc-600">
          <span>You captured: <span className="font-mono font-bold text-arc-900">{capturedDark}</span></span>
          <span>AI captured: <span className="font-mono font-bold text-arc-900">{capturedLight}</span></span>
      ...
      <div className="flex w-full max-w-lg items-center justify-between rounded-xl bg-white px-5 py-3 ring-1 ring-inset ring-purple-200 shadow-sm">
        <div className="flex flex-col gap-1">
          <p className="text-xs font-medium text-arc-500">Your pieces captured by AI</p>
          <div className="flex gap-1">
            ...
            {capturedLight === 0 && <span className="text-xs text-arc-400 italic">none yet</span>}
          </div>
        </div>
        <div className="flex flex-col items-end gap-1">
          <p className="text-xs font-medium text-arc-500">AI pieces captured by you</p>
          <div className="flex gap-1 flex-wrap justify-end">
            ...
            {capturedDark === 0 && <span className="text-xs text-arc-400 italic">none yet</span>}
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="rounded-lg bg-purple-100 px-5 py-2 text-sm font-semibold text-arc-700 ring-1 ring-inset ring-purple-200 shadow-sm transition hover:bg-purple-200 hover:text-arc-900"
      >
        New Game
      </button>

      {/* Legend */}
      <div className="flex gap-6 text-xs text-arc-500">
