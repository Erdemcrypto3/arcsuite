    ...
    <div className="flex flex-col items-center gap-8">
      <div className="flex items-center gap-8">
        ...
        <div className="text-right">
          <div className="text-xs uppercase tracking-wider text-arc-500">Score</div>
          <div className="font-mono text-3xl font-semibold text-arc-900">{score}</div>
          <div className="mt-2 text-xs text-arc-500">Wrong: {wrongCount}/{MAX_WRONG}</div>
        </div>
      </div>
      ...
      <div className="flex gap-2 text-3xl font-mono tracking-widest">
        {word.split('').map((letter, i) => (
          <span key={i} className={`inline-block w-8 border-b-2 text-center ${
            guessed.has(letter) ? 'text-arc-900 border-emerald-400' : gameOver ? 'text-red-500 border-red-400' : 'text-transparent border-purple-300'
          }`}>
          ...
          return (
            <button
              ...
              className={`h-10 w-10 rounded-lg text-sm font-semibold transition ${
                ...
                isGuessed || gameOver ? 'bg-purple-50/50 text-arc-300 cursor-not-allowed' :
                'bg-white text-arc-900 ring-1 ring-purple-200 hover:bg-purple-100 hover:ring-arc-500'
              }`}
            >
              {letter}
            </button>
      ...
      {gameOver && (
        <>
          ...
          <button
            onClick={newGame}
            className="rounded-lg bg-arc-500 px-6 py-2.5 text-sm font-semibold text-white hover:bg-arc-700"
          >
...
function HangmanSvg({ wrongCount }: { wrongCount: number }) {
  return (
    <svg viewBox="0 0 120 140" className="h-36 w-28">
      {/* gallows */}
      <line x1="10" y1="135" x2="70" y2="135" stroke="#0066FF" strokeWidth="3" />
      <line x1="30" y1="135" x2="30" y2="10" stroke="#0066FF" strokeWidth="3" />
      <line x1="30" y1="10" x2="80" y2="10" stroke="#0066FF" strokeWidth="3" />
      <line x1="80" y1="10" x2="80" y2="30" stroke="#0066FF" strokeWidth="3" />
      {/* head */}
