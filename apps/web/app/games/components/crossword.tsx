...
export function Crossword() {
  ...
  const cellBg = (row: number, col: number): string => {
    if (puzzle.grid[row][col] === null) return 'bg-arc-900';
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="text-xs uppercase tracking-wider text-arc-500">Puzzles Solved</div>
          <div className="font-mono text-2xl font-semibold text-arc-900">{completed}</div>
        </div>
        <div className="text-center">
          <div className="text-sm font-semibold text-arc-700">{puzzle.title}</div>
          <div className="text-xs text-arc-400">#{puzzle.id}</div>
        </div>
        <div className="text-right">
          <div className="text-xs uppercase tracking-wider text-arc-500">Best</div>
          <div className="font-mono text-2xl font-semibold text-arc-900">{highScore}</div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
        ...
        <div className="flex-shrink-0 overflow-x-auto">
              ...
              Array.from({ length: cols }, (_, col) => {
                ...
                if (isBlack) {
                  return (
                    <div
                      key={`${row}-${col}`}
                      className="bg-arc-900 rounded-sm"
                ...
                return (
                  ...
                  >
                    {num !== null && (
                      <span className="absolute left-0.5 top-0.5 text-[7px] font-semibold leading-none text-arc-500 select-none">
                        {num}
                      </span>
                    )}
                    <input
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button
              onClick={handleCheck}
              disabled={revealed}
              className="rounded-lg bg-arc-500 px-4 py-2 text-sm font-semibold text-white hover:bg-arc-700 disabled:opacity-50 disabled:cursor-not-allowed transition"
            >
              Check
            </button>
            <button
              ...
              disabled={revealed}
              className="rounded-lg bg-purple-100 px-4 py-2 text-sm font-semibold text-arc-700 ring-1 ring-inset ring-purple-200 hover:bg-purple-200 disabled:opacity-50 disabled:cursor-not-allowed transition"
            >
              Reveal
            </button>
            <button
              onClick={handleNewPuzzle}
              className="rounded-lg bg-purple-100 px-4 py-2 text-sm font-semibold text-arc-700 ring-1 ring-inset ring-purple-200 hover:bg-purple-200 transition"
            >
              New Puzzle
            </button>
          </div>
        ...
        <div className="flex-1 space-y-4 min-w-0 sm:max-h-[30rem] sm:overflow-y-auto sm:pr-2">
          {/* Across clues */}
          <div>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-arc-500">Across</h3>
                  ...
                  <li key={`across-${clue.number}`}>
                    <button
                      ...
                      className={`w-full rounded-md px-2 py-1 text-left text-sm transition ${
                        isActive
                          ? 'bg-purple-100 font-semibold text-arc-900'
                          : 'text-arc-600 hover:bg-purple-50 hover:text-arc-900'
                      }`}
                    >
                      <span className="mr-1.5 font-mono text-xs text-arc-500">{clue.number}.</span>
                      {clue.text}
                    </button>
                  </li>
                );
          ...
          <div>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-arc-500">Down</h3>
            <ul className="space-y-1">
          ...
          {activeClue && (
            <div className="rounded-lg bg-purple-50 px-3 py-2 ring-1 ring-inset ring-purple-200">
              <div className="text-xs text-arc-500 uppercase tracking-wider mb-0.5">
                {activeClue.number} {activeClue.direction}
              </div>
              <div className="text-sm font-medium text-arc-900">
                {(activeClue.direction === 'across'
                  ? puzzle.clues.across
                  : puzzle.clues.down
