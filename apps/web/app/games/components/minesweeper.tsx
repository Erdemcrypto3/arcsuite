...
export function Minesweeper() {
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="flex items-center gap-6 text-sm text-arc-700">
        <span>Mines: <span className="font-mono text-arc-900">{MINES - flagCount}</span></span>
      ...
      {!board && (
        <p className="text-sm text-arc-500">Click any cell to start. Right-click to flag.</p>
      )}
      ...
      {(status === 'won' || status === 'lost') && (
        <button onClick={reset} className="rounded-lg bg-arc-500 px-6 py-2.5 text-sm font-semibold text-white hover:bg-arc-700">
          New Game
        </button>
      )}
    </div>
