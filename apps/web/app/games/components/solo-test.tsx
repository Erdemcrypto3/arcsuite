...
export function SoloTest() {
  ...
  return (
    <div className="flex flex-col items-center gap-4 select-none">
      {/* Header */}
      <div className="flex items-center gap-6 text-sm font-medium text-arc-700">
        <span>Moves: <span className="font-bold text-arc-900">{moveCount}</span></span>
        <span>Pegs left: <span className="font-bold text-arc-900">{pegsLeft}</span></span>
        {highScore !== null && (
          <span>Best: <span className="font-bold text-arc-900">{highScore} peg{highScore !== 1 ? 's' : ''}</span></span>
        )}
      </div>
      ...
      <div className="flex gap-3">
        <button
          ...
          className="rounded-lg border border-arc-200 bg-white px-4 py-2 text-sm font-medium text-arc-700 shadow-sm transition hover:bg-arc-50 disabled:cursor-not-allowed disabled:opacity-40"
        >
          ↩ Undo
        </button>
        <button
          onClick={handleReset}
          className="rounded-lg border border-arc-200 bg-white px-4 py-2 text-sm font-medium text-arc-700 shadow-sm transition hover:bg-arc-50"
        >
          ↺ Reset
        </button>
      </div>
      ...
      {/* Instructions */}
      <p className="text-xs text-arc-500 text-center max-w-xs">
