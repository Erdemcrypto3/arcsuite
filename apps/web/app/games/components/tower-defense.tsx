...
const TOWER_DEFS: Record<TowerType, { name: string; cost: number; damage: number; range: number; rate: number; color: string; aoe: boolean }> = {
  firewall:  { name: 'Firewall',   cost: 50,  damage: 10, range: 2.5, rate: 1.0, color: '#0066FF', aoe: false },
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      for (const b of s.bullets) {
        ...
        ctx.fill();
        ctx.strokeStyle = b.aoe ? '#ef4444' : '#0066FF';
        ctx.lineWidth = 1.5;
        ctx.stroke();
      }
      ...
      if (s.phase === 'prep' && s.wave === 0) {
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('Defend the Arc Network!', W / 2, H / 2 - 20);
        ctx.font = '14px sans-serif';
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between rounded-xl bg-white p-3 ring-1 ring-inset ring-purple-100 shadow-sm text-sm">
        <div className="flex gap-4">
          <span className="font-semibold text-arc-900">Wave: <span className="font-mono text-arc-500">{wave}/15</span></span>
          <span className="font-semibold text-arc-900">Gold: <span className="font-mono text-amber-600">{gold}</span></span>
          <span className="font-semibold text-arc-900">Lives: <span className={`font-mono ${lives <= 5 ? 'text-red-600' : 'text-emerald-600'}`}>{lives}</span></span>
          <span className="font-semibold text-arc-900">Score: <span className="font-mono text-arc-500">{score}</span></span>
        </div>
        {(phase === 'gameover' || phase === 'win') && (
          <div className="flex items-center gap-2">
            <MintScoreNFTButton gameId="towerdefense" gameTitle="Tower Defense" gameIcon="🏰" score={score} />
            <button onClick={restart} className="rounded-lg bg-arc-500 px-4 py-1.5 text-xs font-semibold text-white hover:bg-arc-600">
              Play Again
            </button>
          </div>
        )}
        ...
        {(Object.entries(TOWER_DEFS) as [TowerType, typeof TOWER_DEFS[TowerType]][]).map(([type, def]) => (
          <button
            ...
            className={`flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold ring-1 ring-inset transition ${
              selectedTower === type
                ? 'bg-arc-500 text-white ring-arc-600'
                : gold >= def.cost
                ? 'bg-white text-arc-700 ring-purple-200 hover:ring-arc-500'
                : 'bg-white text-arc-300 ring-purple-100 cursor-not-allowed'
            }`}
          >
      ...
      {/* Legend */}
      <div className="flex flex-wrap gap-4 text-[10px] text-arc-500">
