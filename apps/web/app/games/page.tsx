...
const games: GameInfo[] = [
  ...
  { id: 'towerdefense', icon: '🏰', title: 'Tower Defense', description: 'Defend the Arc network from spam txs, bots, and MEV attackers. 15 waves, 4 tower types.', status: 'live' },
];
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      const metadata = {
        ...
        description: `${gameIcon} ${gameTitle} — Score: ${score} by ${shortAddr} on Arc Game Hub`,
        attributes: [
    ...
    <main className="mx-auto max-w-5xl px-6 py-16 sm:py-20">
      <header className="mb-12">
        <div className="mb-6 flex items-center justify-between">
          <a href="https://arcsuite.xyz" className="inline-flex items-center gap-2 rounded-lg bg-purple-100 px-4 py-2 text-sm font-semibold text-arc-700 ring-1 ring-inset ring-purple-200 shadow-sm transition hover:bg-purple-200 hover:text-arc-900">
            ← arcsuite.xyz
          </a>
          <ConnectButton showBalance={false} />
        </div>
        <h1 className="text-3xl font-semibold tracking-tight text-arc-900 sm:text-5xl">Game Hub</h1>
          ...
          return (
            <div
              ...
              className={`group rounded-xl bg-white p-6 text-left ring-1 ring-inset ring-purple-100 shadow-sm transition ${
                g.status === 'live' ? 'hover:bg-purple-50 hover:ring-arc-500 cursor-pointer' : 'opacity-50 cursor-not-allowed'
              }`}
            >
              <div className="mb-2 flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <span className="text-2xl">{g.icon}</span>
                  <h3 className="text-lg font-semibold text-arc-900">{g.title}</h3>
                </div>
                <span className={`rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${
                  g.status === 'live' ? 'bg-emerald-500/20 text-emerald-300 ring-emerald-500/40' : 'bg-purple-50/50 text-arc-500 ring-purple-100'
                }`}>
                  {g.status === 'live' ? 'Play' : 'Soon'}
                </span>
              </div>
              <p className="mb-4 text-sm text-arc-700">{g.description}</p>
              {hs !== null && (
                <div className="flex items-center justify-between">
                  <div className="text-xs text-arc-500">
                    Best: <span className="font-mono text-arc-500">{hs}</span>
                  </div>
...
function GameWrapper({ title, onBack, children }: { title: string; onBack: () => void; children: React.ReactNode }) {
  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      <div className="mb-6 flex items-center justify-between">
        <button onClick={onBack} className="inline-flex items-center gap-2 rounded-lg bg-purple-100 px-4 py-2 text-sm font-semibold text-arc-700 ring-1 ring-inset ring-purple-200 shadow-sm transition hover:bg-purple-200 hover:text-arc-900">
          ← Back to Game Hub
        </button>
        <ConnectButton showBalance={false} />
      </div>
      <h1 className="mb-6 text-2xl font-semibold text-arc-900">{title}</h1>
      {children}
    </main>
  );
}
